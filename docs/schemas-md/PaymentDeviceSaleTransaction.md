...
| *description*:   | *Request to create sale transaction using decrypted card details from payment device.  <br/> The transaction origin will be fixed as PHONE when data is manually entered into the device and as RETAIL when data is entered into the device via swipe.*|
|----|----|
| requestType |    ``` string ```   <br/> *example: PaymentDeviceSaleTransaction* <br/> Object name of the primary transaction request.|
